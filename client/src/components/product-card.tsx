  ...
  return (
    <Card className="overflow-hidden flex flex-col h-full border border-slate-100 shadow-sm bg-white rounded-xl">
      <div className="relative aspect-square bg-slate-50">
        <img src={activePhoto} className="object-cover w-full h-full transition-all duration-300" />
        ...
        {allPhotos.length > 1 && (
          <div className="absolute bottom-2 left-2 right-2 flex gap-1 overflow-x-auto no-scrollbar">
            {allPhotos.map((img, i) => (
              <img key={i} src={img} onClick={() => setActivePhoto(img)} className={`w-8 h-8 rounded border-2 cursor-pointer ${activePhoto === img ? "border-blue-600" : "border-white"}`} />
            ))}
          </div>
        )}
      </div>
      ...
      <CardContent className="p-2.5 flex-grow space-y-2">
        ...
        <div className="flex items-center justify-between text-[10px]">
          <span className="text-slate-400 uppercase font-bold">Мин. заказ:</span>
          <span className="font-black text-blue-600">{step} пар</span>
        </div>
      </CardContent>
      ...
      <CardFooter className="p-2.5 pt-0 flex flex-col gap-2">
        ...
        {quantity === 0 ? (
          <Button className="w-full h-8 bg-blue-600 text-white font-bold text-[10px]" onClick={() => addItem(product, step)}>В КОРЗИНУ</Button>
        ) : (
          <div className="flex items-center justify-between w-full bg-slate-50 p-0.5 rounded-lg border">
